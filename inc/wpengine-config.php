...
class WPEngine_Config {
	...
	public static function init() {
		add_action( 'init', array( __CLASS__, 'setup_cache_headers' ) );
		add_action( 'wp_enqueue_scripts', array( __CLASS__, 'setup_cdn_assets' ) );
		add_action( 'admin_init', array( __CLASS__, 'setup_object_cache' ) );
	...
	public static function setup_cdn_assets() {
		...
		add_filter( 'stylesheet_uri', array( __CLASS__, 'cdn_url_filter' ) );
		add_filter( 'script_loader_src', array( __CLASS__, 'cdn_url_filter' ) );
		add_filter( 'wp_get_attachment_url', array( __CLASS__, 'cdn_url_filter' ) );
	}
	...
	public static function setup_object_cache() {
		...
		add_filter( 'posts_clauses', array( __CLASS__, 'extend_cache_timeout' ), 10, 2 );
	}
	...
	public static function add_cache_admin_bar() {
		...
		$wp_admin_bar->add_menu(
			array(
				...
				'meta'  => array( 'title' => 'Clear WPEngine Cache' ),
			)
		);
	}
...
if ( is_admin_bar_showing() ) {
	add_action( 'admin_bar_menu', array( 'WPEngine_Config', 'add_cache_admin_bar' ), 100 );
	add_action( 'admin_post_clear_cache', array( 'WPEngine_Config', 'handle_cache_clear' ) );
}
